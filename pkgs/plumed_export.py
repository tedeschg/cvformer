...
class _FlattenEncoderPlumed(nn.Module):
    ...
    def __init__(self, full_model: nn.Module, n_tokens: int, mask_np: np.ndarray):
        super().__init__()
    ...
    def forward(self, x_flat: torch.Tensor) -> torch.Tensor:
        ...
        return self.full_model.encode(x, mask=self.mask)
...
class _DihedralEncoderFromCoordsPlumed(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.full_model.encode(tok, mask=self.mask)
...
def export_plumed_encoder(
    *,
    ...
    wrapper = _FlattenEncoderPlumed(model_export, n_tokens=int(n_tokens), mask_np=np.asarray(mask_np))
    wrapper.eval()
