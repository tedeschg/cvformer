# ==========================================================
# Dihedral Transformer Autoencoder - VERSION 2.3 (MD-SAFE)
# ==========================================================
# Adds:
# - bounded latent (tanh) for CV stability
# - Jacobian regularization (Hutchinson estimator) to tame |dz/dx|
# - attn pooling temperature (or mean pooling) to avoid sharp weights
# - PLUMED export uses the same constant mask as training (module pkgs/plumed_export.py)
# ==========================================================
...
def main(args):
    ...
    # Constant mask on device (NOT batched) for training
    ...
    print(f"  jacobian_lambda={args.jacobian_lambda}  jacobian_num_probes={args.jacobian_num_probes}")
    print(f"  pool_type={args.pool_type}  attn_temperature={args.attn_temperature}")
    print(f"  latent_bound={args.latent_bound}")
    ...
    for epoch in range(args.epochs):
        tr = train_epoch(
            ...
            jacobian_num_probes=args.jacobian_num_probes,
        )
            ...
            torch.save(
                {
                    ...
                    "mask": mask_np,  # numpy boolean mask (preferred for export)
                    "n_tokens": int(n_tokens),  # helpful for downstream tools
                },
    ...
    # =============================
    # PLUMED EXPORT (MODULE)
    # =============================
    print(f"\n{'=' * 60}")
    print("Exporting model for PLUMED integration (mask-aware module)...")
    print(f"{'=' * 60}")

    # IMPORTANT: export from CPU eval copy
    # Keep model on GPU for possible further work; export module will move a copy to CPU.
    model_for_export = DihedralTransformerAE(
        n_tokens=n_tokens,
        d_model=args.d_model,
        nhead=args.nhead,
        num_encoder_layers=args.num_encoder_layers,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Dihedral Transformer Autoencoder (MD-safe latents + Jacobian reg)",
    ...
    # Pooling / stability knobs
    parser.add_argument("--pool_type", type=str, default="attn", choices=["attn", "mean"], help="Pooling type")
    parser.add_argument("--attn_temperature", type=float, default=2.0, help="Softmax temperature (>1 softer)")
    parser.add_argument("--latent_bound", type=float, default=2.0, help="0 disables; >0 bounds latent via tanh")
    ...
    # Jacobian regularization (MD-oriented)
    parser.add_argument("--jacobian_lambda", type=float, default=1e-3, help="0 disables; strength of Jacobian penalty")
    parser.add_argument("--jacobian_num_probes", type=int, default=1, help="Hutchinson probes per batch (1-2 typical)")
